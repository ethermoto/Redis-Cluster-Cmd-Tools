...
class RedisTrib
  ...
  def load_cluster_info_from_node(nodeaddr)
    ...
    node.setPassword($clusterPassword)
    ...
    node.friends.each { |f|
      ...
      fnode.setPassword($clusterPassword)
      fnode.connect()
      next if !fnode.r
      begin
        fnode.load_info()
  ...
  def addnode_cluster_cmd(argv, opt)
    ...
    new.setPassword($clusterPassword)
...
if !cmd_spec
  puts "Unknown redis-trib subcommand '#{$cmdBeginIndex}'"
...
# Parse options
cmd_options, first_non_option = rt.parse_options(ARGV[$cmdBeginIndex].downcase)
